...
const nftStorageClient = new NFTStorage({ token: nftStorageKey });
...
export default function NFTUpload({ nftStored, setNftStored }) {
  ...
  const submitNFT = async () => {
    if (uploadedFile) {
      ...
      notification["info"]({
        ...
        description: <>Currently uploading and pinning. This may take >1m, please be patient</>,
      });
      try {
        ...
        notification["success"]({
          ...
          key,
          message: "NFT pinned",
          description: <>Hosted via NFT.Storage at {metadata.url}</>,
        });
  ...
  return (
    <>
      <Dragger action="" beforeUpload={onUpload}>
